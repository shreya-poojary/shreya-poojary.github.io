...
export function Experience() {
  return (
    <section id="experience" className="py-20 bg-gradient-to-b from-slate-950 to-slate-900 relative overflow-hidden scroll-mt-20">
          ...
          {experiences.map((exp, idx) => (
            ...
            >
              ...
              {idx < experiences.length - 1 && (
                <div className={`absolute left-6 top-16 w-0.5 h-full ${colorClasses[exp.color].glow} ${colorClasses[exp.color].border} border-l-2`} />
              )}
              ...
              <div className="flex gap-6">
                ...
                <motion.div
                  ...
                  className={`relative z-10 flex-shrink-0 w-12 h-12 rounded-full ${colorClasses[exp.color].glow} border-2 ${colorClasses[exp.color].border} flex items-center justify-center`}
                >
                  <Briefcase className={`w-5 h-5 ${colorClasses[exp.color].text}`} />
                </motion.div>
                ...
                >
                  <div className="p-6 bg-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-lg hover:border-slate-600 transition-colors">
                    <div className="mb-4">
                      <h3 className={`text-2xl ${colorClasses[exp.color].text} mb-1`}>
                        {exp.title}
                      </h3>
                    ...
                    <ul className="space-y-2">
                      {exp.highlights.map((highlight, hIdx) => (
                        <li key={hIdx} className="text-slate-400 flex gap-2">
                          <span className={`${colorClasses[exp.color].text} flex-shrink-0`}>▹</span>
                          <span>{highlight}</span>
                        </li>
                      ))}
                    </ul>
